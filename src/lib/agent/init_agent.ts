...
import { InitAgentState, llm, langfuseHandler } from './langgraph.js';
...
async function profilerNode(state: typeof InitAgentState.State): Promise<Partial<typeof InitAgentState.State>> {
  const { casts } = state;
  ...
  try {
    ...
    if (extractedJSON) {
      console.log('[ProfilerNode] Successfully extracted JSON from first attempt.');
      return {
    ...
    console.log('[ProfilerNode] JSON extraction failed. Retrying with more explicit prompt...');
    ...
    try {
      ...
      if (extractedJSON) {
        console.log('[ProfilerNode] Successfully extracted JSON from retry attempt.');
        return {
          response: extractedJSON
        };
      }
      ...
      console.log('[ProfilerNode] JSON extraction failed on retry. Using standard response.');
      return {
...
export async function runInitAgent(
  ...
  options?: { isFilePath?: boolean }
    ...
    try {
      ...
      console.log(`[InitAgent] Processing data with ${processedData.length} characters`);
