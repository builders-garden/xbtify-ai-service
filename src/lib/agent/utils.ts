...
export function extractJSON(text: string): string | null {
    ...
    try {
      ...
      try {
        ...
        if (parsed.extractedData && Array.isArray(parsed.extractedData)) {
          ...
          throw new Error('Invalid data format: expected array or object with extractedData property');
        }
      } catch (parseError) {
      ...
      console.log(`[PreProcessor] Extracted ${extractedTexts.length} text entries`);
  ...
  // Pre-processing function to extract reply fields from a file
  export function preprocessRepliesFile(filePath: string): string {
    try {
      console.log(`[PreProcessor] Reading replies file: ${filePath}`);
      ...
      // Extract only the parentText and text fields from each object
      const extractedReplies = data.map((item: any) => ({
        parentText: item.parentText,
        text: item.text
      }));
      
      console.log(`[PreProcessor] Extracted ${extractedReplies.length} reply entries`);
      
      // Return as JSON string
      return JSON.stringify(extractedReplies);
    } catch (error) {
      console.error('[PreProcessor] Error processing replies file:', error);
      throw error;
    }
  }
