...
export function chunkCasts(casts: Cast[], creatorFid: number): ChunkData[] {
	...
	for (const cast of casts) {
		...
		if (
			currentSize + castLength + CAST_SEPARATOR.length > MAX_CHUNK_SIZE &&
			...
			currentSize +=
				castLength + (currentChunk.length > 1 ? CAST_SEPARATOR.length : 0);
		}
	}
	...
	console.log(
		`[chunking] Created ${chunks.length} chunks from ${casts.length} casts`,
	);
	return chunks;
}
